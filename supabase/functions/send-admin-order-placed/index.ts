...
function buildEmailHtml(order: any): string {
  ...
  const notes = safe(order.customerInfo?.notes ?? order.notes ?? "");
...
serve(async (req) => {
  ...
  try {
    ...
    console.log("send-admin-order-placed received:", { orderNumber, customerName });
    ...
    console.log("Admin email (order placed) sent via Zoho SMTP");
